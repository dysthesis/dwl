...
#define SVISIBLEON(C, M)                                                       \
  ((M) && (C)->mon && ((C)->tags & (M)->tagset[(M)->seltags]))
...
/* When a client is swallowing another, optionally add extra border width */
#define BORDERPX(C)                                                            \
  (borderpx +                                                                  \
...
typedef struct {
  ...
  int noswallow;
  int monitor;
  /* key to group/identify scratchpad windows; 0 means none */
  char scratchkey;
  int x;
  int y;
  float w;
  ...
  for (r = rules; r < END(rules); r++) {
    if ((!r->title || strstr(title, r->title)) &&
      ...
      if (c->isfloating || !mon->lt[mon->sellt]->arrange) {
        ...
        newwidth = (int)round((r->w > 0) ? ((r->w <= 1) ? b.width * r->w : r->w)
                                         : c->geom.width);
        newheight =
            (int)round((r->h > 0) ? ((r->h <= 1) ? b.height * r->h : r->h)
                                  : c->geom.height);
...
void bufdataend(struct wlr_buffer *wlr_buffer) { (void)wlr_buffer; }
...
void bufrelease(struct wl_listener *listener, void *data) {
  Buffer *buf;
  (void)data;
  buf = wl_container_of(listener, buf, release);
  buf->busy = false;
  wl_list_remove(&buf->release.link);
}
...
unsigned int getpertagtag(unsigned int curtagset) {
  size_t i;
...
/* removed printstatus: switched to in-process bar drawing */
...
void setlayout(const Arg *arg) {
  ...
  if (arg && arg->v)
    selmon->lt[selmon->sellt] =
        pertag.ltidxs[selmon->pertag[selmon->seltags]][selmon->sellt] =
            (Layout *)arg->v;
    ...
    if (i < m->nmaster) {
      ...
      resize(c,
             (struct wlr_box){.x = m->w.x,
                              .y = m->w.y + my,
                              .width = mw,
                              .height = (m->w.height - my) / denom_master},
             0, draw_borders);
      my += c->geom.height;
    } else {
      int denom_stack = n - i;
      ...
      resize(c,
             (struct wlr_box){.x = m->w.x + mw,
                              .y = m->w.y + ty,
                              .width = m->w.width - mw,
                              .height = (m->w.height - ty) / denom_stack},
             0, draw_borders);
      ty += c->geom.height;
    }
    i++;
...
static void __attribute__((unused)) togglebar(const Arg *arg) {
  (void)arg;
  if (!selmon || !selmon->scene_buffer)
    return;
...
void toggletag(const Arg *arg) {
  ...
  wl_list_for_each(m, &mons, link) if (m != selmon &&
                                       newtags & m->tagset[m->seltags]) return;
...
void toggleview(const Arg *arg) {
  ...
  wl_list_for_each(m, &mons,
                   link) if (m != selmon &&
                             newtagset & m->tagset[m->seltags]) return;

  // set new pertag tag only if the tag we were at was removed, or if all tags
  // are shown.
  if (!(newtagset & 1 << (selmon->pertag[selmon->seltags] - 1)) ||
      newtagset == TAGMASK) {
